...
class LinearOperatorLowRankUpdate(linear_operator.LinearOperator):
  ...
  `x` having `_ops.TensorShape(x.shape) = [N, R]` with `O(L_matmul*N*R)` complexity (and similarly
  for `solve`, `determinant`.  Then, if `_ops.TensorShape(x.shape) = [N, R]`,
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _check_shapes(self):
    ...
    uv_shape = _ops.broadcast_static_shape_as_tensorshape(
        _ops.TensorShape(self.u.shape), _ops.TensorShape(self.v.shape))

    batch_shape = _ops.broadcast_static_shape_as_tensorshape(
    ...
    if self._diag_update is not None:
      tensor_shape.dimension_at_index(uv_shape, -1).assert_is_compatible_with(
          _ops.TensorShape(self._diag_update.shape)[-1])
      _ops.broadcast_static_shape_as_tensorshape(
          batch_shape, _ops.TensorShape(self._diag_update.shape)[:-1])
  ...
  def _set_diag_operators(self, diag_update, is_diag_update_positive):
    ...
    if diag_update is not None:
      ...
      if tensor_shape.dimension_value(_ops.TensorShape(self.u.shape)[-1]) is not None:
        r = tensor_shape.dimension_value(_ops.TensorShape(self.u.shape)[-1])
      else:
  ...
  def _shape(self):
    batch_shape = _ops.broadcast_static_shape_as_tensorshape(
        ...
        _ops.TensorShape(self.u.shape)[:-2])
    return batch_shape.concatenate(_ops.TensorShape(self.base_operator.shape)[-2:])
