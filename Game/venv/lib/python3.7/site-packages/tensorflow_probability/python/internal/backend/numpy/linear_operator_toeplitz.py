  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _check_row_col(self, row, col):
    ...
    for name, tensor in [["row", row], ["col", col]]:
      if _ops.TensorShape(tensor.shape).ndims is not None and _ops.TensorShape(tensor.shape).ndims < 1:
        raise ValueError("Argument {} must have at least 1 dimension.  "
                         "Found: {}".format(name, tensor))

    if _ops.TensorShape(row.shape)[-1] is not None and _ops.TensorShape(col.shape)[-1] is not None:
      if _ops.TensorShape(row.shape)[-1] != _ops.TensorShape(col.shape)[-1]:
        raise ValueError(
  ...
  def _shape(self):
    ...
    v_shape = _ops.broadcast_static_shape_as_tensorshape(
        _ops.TensorShape(self.row.shape), _ops.TensorShape(self.col.shape))
