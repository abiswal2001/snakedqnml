...
def minimize(objective_function,
  ...
  ```python
    ...
    initial_population = (tf.random.stateless_normal([population_size]),
                          tf.random.stateless_normal([population_size]))
    def easom_fn(x, y):
...
def _get_starting_population(initial_population,
  ...
  for part in initial_position:
    ...
    population_part = tf.random.stateless_normal(population_part_shape,
...
def _binary_crossover(population,
  ...
  for i, population_part in enumerate(population):
    ...
    force_crossovers = tf.one_hot(
        tf.random.stateless_uniform([population_size],
                          minval=0,
                          maxval=part_size,
                          dtype=tf.int32,
                          seed=seed_stream()),
    ...
    do_binary_crossover = tf.random.stateless_uniform(
...
def _get_mixing_indices(size, seed=None, name=None):
  ...
  with tf.name_scope(name or 'get_mixing_indices'):
    ...
    first = tf.random.stateless_uniform([size],
                              maxval=size-1,
                              dtype=dtype,
                              seed=seed_stream())
    second = tf.random.stateless_uniform([size],
                               maxval=size-2,
                               dtype=dtype,
                               seed=seed_stream())
    third = tf.random.stateless_uniform([size],
                              maxval=size-3,
                              dtype=dtype,
                              seed=seed_stream())
